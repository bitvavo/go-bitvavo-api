...
func updateRateLimit(response http.Header) {
	// Possible errors of parsing integers are ignored. I decided
	// to do this because the methods using this function don't
	// handle errors, and the methods using those functions
	// aren't setup to handle errors of any other type than JSON
	// errors returned by the Bitvavo API.
	//
	// Changing all this would be a nice in the future, but in
	// that case it might be worth rewriting a lot more of this
	// client library.
	remaining, _ := strconv.ParseInt(response.Get("bitvavo-ratelimit-remaining"), 10, 64)
	rateLimitRemaining = int(remaining)
	reset, _ := strconv.ParseInt(response.Get("bitvavo-ratelimit-resetat"), 10, 64)
	rateLimitReset = int(reset)

	millisecondsToWait := reset - time.Now().UnixMilli()
	time.AfterFunc(time.Duration(millisecondsToWait)*time.Millisecond, checkLimit)
}
